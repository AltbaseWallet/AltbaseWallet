  ...
  async buildItems(coins: Coin[]): Promise<WalletSnapshotItem[]> {
    ...
    const items = await Promise.all(
      coins.map(async (coin) => {
        ...
        try {
          // Use the engine's network-facing address. BCH2's RPC indexes can
          // disagree across aliases; replacing its cashaddr with legacy here
          // silently turns a funded receive address into an empty balance.
